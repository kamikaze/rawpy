...
from rawpy.enhance import _repair_bad_pixels_bayer2x2, _repair_bad_pixels_generic, find_bad_pixels

THIS_DIR = os.path.dirname(__file__)

# Nikon D3S
RAW_TEST_PATH = os.path.join(THIS_DIR, 'iss030e122639.NEF')
BAD_PIXELS_TEST_PATH = os.path.join(THIS_DIR, 'bad_pixels.gz')

# Nikon D4
RAW2_TEST_PATH = os.path.join(THIS_DIR, 'iss042e297200.NEF')

# Canon EOS 5D Mark 2
RAW3_TEST_PATH = os.path.join(THIS_DIR, 'RAW_CANON_5DMARK2_PREPROD.CR2')

# Sigma SD9 (Foveon)
RAW4_TEST_PATH = os.path.join(THIS_DIR, 'RAW_SIGMA_SD9_SRGB.X3F')

# Canon 40D in sRAW format with 4 channels
RAW5_TEST_PATH = os.path.join(THIS_DIR, 'RAW_CANON_40D_SRAW_V103.CR2')

# Kodak DC50 with special characters in filename
RAW6_TEST_PATH = os.path.join(THIS_DIR, 'RAW_KODAK_DC50_é.KDC')

# Sony A7RIV ARW file
RAW7_TEST_PATH = os.path.join(THIS_DIR, 'RAW_SONY_A7RIV.ARW')
RAW7_CAMERA_PROFILE_PATH = os.path.join(THIS_DIR, 'SONY_ILCE-7RM4_iso100_FE_70-200mm_F2.8_GM_OSS_f5.6_daylight.dcp')
RAW7_SRGB_OUTPUT_PROFILE_PATH = os.path.join(THIS_DIR, 'sRGB.icm')
RAW7_ADOBE_RGB_OUTPUT_PROFILE_PATH = os.path.join(THIS_DIR, 'AdobeRGB.icm')
RAW7_PRO_PHOTO_OUTPUT_PROFILE_PATH = os.path.join(THIS_DIR, 'ProPhoto.icm')
...
def test_version():
    ...
    for d in rawpy.DemosaicAlgorithm:
        print(d.name, 'NOT' if d.isSupported is False else 'possibly' if d.isSupported is None else '', 'supported')


def test_file_open_and_post_process():
    raw = rawpy.imread(RAW_TEST_PATH)
    ...
    rgb = raw.postprocess(no_auto_bright=True, user_wb=raw.daylight_whitebalance, output_bps=16)
    print_stats(rgb)
    ...
    rgb = raw.postprocess(no_auto_bright=True, user_wb=raw.daylight_whitebalance, gamma=(1, 1), output_bps=16)
    print_stats(rgb)
    save('test_16daylight_linear.tiff', rgb)


def test_camera_and_output_profiles():
    raw = rawpy.imread(RAW7_TEST_PATH)

    params = Params(
        median_filter_passes=0, use_camera_wb=True, output_bps=16, no_auto_bright=True, output_tiff=True,
        camera_profile=RAW7_CAMERA_PROFILE_PATH,
        output_profile=RAW7_SRGB_OUTPUT_PROFILE_PATH
    )
    rgb = raw.postprocess(params=params)
    print_stats(rgb)
    save('test7_profiles_srgb16.tiff', rgb)

    params = Params(
        median_filter_passes=0, use_camera_wb=True, output_bps=16, no_auto_bright=True, output_tiff=True,
        camera_profile=RAW7_CAMERA_PROFILE_PATH,
        output_profile=RAW7_ADOBE_RGB_OUTPUT_PROFILE_PATH
    )
    rgb = raw.postprocess(params=params)
    print_stats(rgb)
    save('test7_profiles_adobergb16.tiff', rgb)

    params = Params(
        median_filter_passes=0, use_camera_wb=True, output_bps=16, no_auto_bright=True, output_tiff=True,
        camera_profile=RAW7_CAMERA_PROFILE_PATH,
        output_profile=RAW7_PRO_PHOTO_OUTPUT_PROFILE_PATH
    )
    rgb = raw.postprocess(params=params)
    print_stats(rgb)
    save('test7_profiles_prophoto16.tiff', rgb)
    ...
    rgb = raw.postprocess(params=params)
    print_stats(rgb)
    save('test7_no_profile16.tiff', rgb)


def test_foveon_file_open_and_post_process():
    raw = rawpy.imread(RAW4_TEST_PATH)
...
def test_sraw_file_open_and_post_process():
    raw = rawpy.imread(RAW5_TEST_PATH)
...
def test_file_open_with_non_ascii_characters():
    rawpy.imread(RAW6_TEST_PATH)


def test_buffer_open():
    with open(RAW_TEST_PATH, 'rb') as rawfile:
...
def test_context_manager():
    with rawpy.imread(RAW_TEST_PATH) as raw:
        assert_array_equal(raw.raw_image.shape, [2844, 4288])
...
def test_windows_file_lock_release():
    # see https://github.com/neothemachine/rawpy/issues/10
    # we make a copy of the raw file which we will later remove
    copy_path = RAW_TEST_PATH + '-copy'
    shutil.copyfile(RAW_TEST_PATH, copy_path)

    with rawpy.imread(copy_path) as raw:
        rgb = raw.postprocess()
...
def test_thumb_extract_bitmap():
    with rawpy.imread(RAW4_TEST_PATH) as raw:
        thumb = raw.extract_thumb()
...
def test_bad_pixel_repair():
    ...
    bad_pixels = np.loadtxt(BAD_PIXELS_TEST_PATH, int)
    i = 60
    ...
    for useOpenCV in [False, True]:
        ...
        for repair in [_repair_bad_pixels_generic, _repair_bad_pixels_bayer2x2]:
            ...
            before = get_color_neighbors(raw, y, x)
            repair(raw, bad_pixels, method='median')
            after = get_color_neighbors(raw, y, x)
        ...
        if not useOpenCV:
            rawpy.enhance.cv2 = old_cv


def test_visible_size():
    for path in [RAW_TEST_PATH, RAW2_TEST_PATH]:
        print('testing', path)
        raw = rawpy.imread(path)
        s = raw.sizes
...
def test_highlight_mode_parameter():
    raw = rawpy.imread(RAW_TEST_PATH)
...
def test_segfault_bug():
    # https://github.com/neothemachine/rawpy/issues/7
    im = rawpy.imread(RAW_TEST_PATH).raw_image
    assert_array_equal(im.shape, [2844, 4288])
    print(im)


def test_libraw_file_unsupported_error():
    with pytest.raises(rawpy.LibRawFileUnsupportedError):
        rawpy.imread(os.path.join(THIS_DIR, 'README.txt'))


def test_libraw_out_of_order_call_error():
...
if __name__ == '__main__':
    test_version()
    # test_file_open_and_post_process()
    # test_bad_pixel_repair()
    test_find_bad_pixels_nikon_d4()
